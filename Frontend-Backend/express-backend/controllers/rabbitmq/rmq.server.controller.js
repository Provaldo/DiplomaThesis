...
deploymentCreator = (req, res, next) => {
  ...
  var deploymentObject = {
    ...
    kind: "Deployment",
    metadata: {
      name: `rabbitmq-${req.body.username}`,
      labels: {
        app: "rabbitmq-server",
        tag: `rabbitmq-${req.body.username}`,
        owner: req.body.username,
      },
    },
    spec: {
      selector: {
        matchLabels: { app: `rabbitmq-server-${req.body.username}` },
      },
      template: {
        metadata: {
          name: `rabbitmq-${req.body.username}`,
          labels: { app: `rabbitmq-server-${req.body.username}` },
        },
        spec: {
          containers: [
            {
  ...
  serverExists(namespace, req.body.username).then((exists) => {
    if (exists == 0) {
      k8sApi
